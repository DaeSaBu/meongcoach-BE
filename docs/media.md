...
- 이미지 업로드 URL: `POST /api/media/image-upload-urls` (`MediaController`)
- 영상 업로드 URL: media가 직접 노출하지 않습니다. `ai` 모듈이 `VideoUploadUrlIssuer`(provided)로 발급받아 `POST /api/ai/video-upload-urls`로 노출합니다. 업로드 이후의 비동기 분석 흐름은 [ai-pipeline.md](ai-pipeline.md)를 참고하세요.
