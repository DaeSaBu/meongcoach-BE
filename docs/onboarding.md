...
|---|---|---|---|---|
| `priorTrainingTopicIds` | 정수 배열 | 미전송·`null`·빈 배열은 선택 없음 | 최대 100개, 원소는 null이 아닌 양수이자 존재하는 토픽 ID | `user_prior_training_topics` |
| `trainingGoalTopicIds` | 정수 배열 | 미전송·`null`·빈 배열은 선택 없음 | 최대 100개, 원소는 null이 아닌 양수이자 존재하는 토픽 ID | `user_training_goal_topics` |
...
user_prior_training_topics
├─ user_id (PK, FK -> user_profiles.user_id, ON DELETE CASCADE)
└─ topic_id (PK, FK -> topics.id, ON DELETE RESTRICT)

user_training_goal_topics
├─ user_id (PK, FK -> user_profiles.user_id, ON DELETE CASCADE)
└─ topic_id (PK, FK -> topics.id, ON DELETE RESTRICT)
...
`dogs` 행에 저장한다. 토픽 엔티티를 user 모듈에서 직접 참조하지 않고 ID만 보관하며, 모듈 간 검증은 training 모듈의
`TopicValidator` 공개 인터페이스로 수행한다.
...
프로필, 두 토픽 컬렉션과 모든 반려견은 기존 온보딩 쓰기 트랜잭션 안에서 함께 저장된다. 어느 단계에서든 실패하면
