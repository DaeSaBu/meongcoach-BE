...
dev/prod의 스키마는 Flyway가 만듭니다. Hibernate는 `validate`로 대조만 합니다. 프로파일별 설정은
[profiles.md](profiles.md)를 참고하세요.
...
두 브랜치가 각각 같은 `V{n}`을 만들면 머지 후 번호가 겹칩니다. **머지 전에 `develop`의 최신 번호를 다시
...
- 기동 시 마이그레이션은 `spring-boot-starter-flyway`가 수행합니다.
  - Spring Boot 4가 Flyway 자동 구성을 별도 모듈로 분리했습니다. 이 의존성이 없으면 `spring.flyway` 설정이 있어도 오류 없이 실행되지 않습니다.
- dev에는 Flyway 도입 전 `ddl-auto`가 만든 스키마가 남아 있어 `baseline-on-migrate`로 그것을 V1 적용 상태로 기록합니다. baseline이 기록된 뒤 제거합니다.
