...
| 액세스 토큰 | 1시간 |
| 리프레시 토큰 | 14일 |
| 클레임 | `iss`, `sub`(회원 ID), `iat`, `exp`, `token_type`, `jti`(리프레시 토큰만), `email`(액세스 토큰만) |
| 저장 | 액세스 토큰은 저장하지 않고 서명·만료·용도로만 검증. 리프레시 토큰은 원문 대신 `jti`·만료·폐기 시각을 `refresh_tokens`에 저장 |

### email 클레임

클라이언트가 PostHog 같은 분석 도구에 회원 속성으로 넘길 값입니다. 분석 도구의 사용자 축은 `sub`(회원 ID)가 맡고,
이메일은 사람이 알아볼 수 있는 표시용 속성입니다. 조회 API 없이 토큰만으로 읽을 수 있어 로그인 직후 왕복이 필요 없습니다.

- **액세스 토큰에만 싣습니다.** 14일을 사는 리프레시 토큰에 넣으면 얻는 것 없이 개인정보 노출 창만 길어집니다.
- **값은 `AuthTokenIssueService`가 읽습니다.** 소셜 계정(`social_accounts.email`)을 먼저 보고, 없으면 로컬 계정(`local_accounts.email`)을 봅니다.
  로컬 계정은 심사용 테스트 계정 전용이라 한 회원이 둘을 함께 갖지 않습니다.
- **없을 수 있습니다.** 카카오·애플은 동의가 없으면 이메일을 내려주지 않으므로, 그런 회원의 토큰에는 `email` 클레임 자체가 없습니다.
  클라이언트는 클레임이 없는 경우를 정상으로 다뤄야 합니다.
- **서버는 이 클레임을 인가에 쓰지 않습니다.** JWT는 서명만 되고 암호화되지 않으므로 토큰이 로그에 남으면 이메일도 함께 남습니다.
- **이메일 변경은 재발급 시점에 반영됩니다.** 발급마다 계정에서 다시 읽으므로 최대 액세스 토큰 수명(1시간)만큼 낡은 값이 흐를 수 있습니다.
