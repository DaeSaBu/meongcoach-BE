...
| 이메일 형식 오류 | 400 `USER_INVALID_EMAIL` — 계정 존재와 무관한 입력 형식 검증이라 정보가 새지 않습니다. 형식 규칙은 `Email` 값 객체 한 곳에만 두고, 요청 DTO의 `toEmail()`이 변환합니다 |
...
- **소셜 이메일도 `Email` 값 객체로 검증합니다.** 클레임이 없으면 `Email.ofNullable`이 null로 두지만,
  값이 있는데 형식 규칙(ASCII 정규식·255자)을 통과하지 못하면 400 `USER_INVALID_EMAIL`로 **로그인 자체가
  거절됩니다.** 소셜 이메일은 저장만 하고 조회·인증에 쓰지 않으므로, 정상 계정이 이 검증에 막히는 사례가
  나오면 규칙을 넓히거나 리더에서 null로 떨어뜨리는 쪽을 검토합니다. 예외 detail에는 주소를 싣지 않습니다.
