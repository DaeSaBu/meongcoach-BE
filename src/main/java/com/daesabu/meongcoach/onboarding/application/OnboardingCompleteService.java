...
/**
 * 온보딩 완료를 오케스트레이션한다. 입력에 포함된 외부 자원과 토픽을 검증한 뒤,
...
public class OnboardingCompleteService implements OnboardingCompleter {
	...
	private void validateTopicIds(OnboardingCompleteInfo info) {
		Set<Long> topicIds = new HashSet<>(info.priorTrainingTopicIds());
		topicIds.addAll(info.trainingGoalTopicIds());
		topicValidator.validateAllExist(topicIds);
	}
}
