...
import com.daesabu.meongcoach.auth.adapter.webapi.dto.UserWithdrawRequest;
...
public class WithdrawController {
	...
	private final AccountWithdrawer accountWithdrawer;

	// 탈퇴는 자격증명·토큰 정리가 먼저라 auth가 조율하므로, 경로는 /api/users지만 컨트롤러를 auth에 둔다.
	...
	public void withdraw(@CurrentUserId Long userId, @RequestBody(required = false) UserWithdrawRequest request) {
		...
		accountWithdrawer.withdraw(userId, appleAuthorizationCode);
	}
