...
-- 로그인: POST /api/auth/login/email { "email": "review@meongcoach.com", "password": "meongcoach-review" }
-- password_hash는 BCrypt. 재생성: htpasswd -bnBC 10 "" '비밀번호' | tr -d ':\n'  ($2y$ 접두어도 BCryptPasswordEncoder가 허용)
-- 온보딩 전 상태(ONBOARDING_USER, 프로필 없음)로 두어 심사자가 앱에서 온보딩까지 직접 진행한다.
...
INSERT INTO "users" ("id", "role", "status", "created_at", "updated_at") VALUES
	(1, 'ONBOARDING_USER', 'ACTIVE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
ON CONFLICT ("id") DO NOTHING;

INSERT INTO "email_accounts" ("id", "user_id", "email", "password_hash", "created_at", "updated_at") VALUES
...
SELECT setval(pg_get_serial_sequence('email_accounts', 'id'), (SELECT MAX("id") FROM "email_accounts"), true);
