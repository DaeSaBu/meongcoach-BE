...
1. 하나의 의미 단위 작업이 끝나면 즉시 커밋한다. 여러 작업을 모아 통으로 커밋하지 않는다. 동작을 추가·변경하는 작업은 테스트 커밋을 먼저, 구현 커밋을 뒤에 만든다. ([커밋 단위](#커밋-단위) 참고)
...
- 영문 소문자와 하이픈으로 작성한다.
- 이 저장소의 scope 후보는 Spring Modulith 모듈 패키지명과 같다. `auth`(로그인·토큰·자격증명), `user`(회원 역할·상태·프로필), `dog`(반려견 프로필), `training`(훈련 콘텐츠 카탈로그),
  `progress`(학습 진도), `ai`(AI 영상 분석), `onboarding`(온보딩 흐름 조합), `media`(이미지·영상 업로드), `health`(서비스 상태 확인), `purchase`(인앱 구매 기록), `entitlement`(이용권 부여·확인), `shared`(횡단 관심사).
...
- 동작을 추가·변경하는 작업(`feature`, `fix`)은 TDD 순서로 커밋한다 — 검증할 동작을 담은 테스트를 `test` 커밋으로 먼저 올리고, 그 테스트를 통과시키는 구현을 `feature`/`fix` 커밋으로 뒤이어 올린다. 테스트와 구현을 한 커밋에 섞지 않는다.
	- 테스트 커밋에는 테스트 코드만 담는다. 아직 없는 클래스를 참조해 그 커밋 단독으로는 컴파일되지 않아도 된다(의도된 red 상태). 인터페이스 골격 같은 메인 코드를 테스트 커밋에 끼워 넣지 않는다.
	- 테스트 커밋과 구현 커밋은 같은 push로 올린다. pre-push 훅은 포맷(spotless)만 검사하고 CI는 PR head 기준이라 빌드에는 영향이 없지만, `git bisect`에서는 red 커밋을 건너뛴다.
	- 기존 테스트만 보강·수정하는 작업은 `test` 단독 커밋이다. `refactor`는 동작이 바뀌지 않으므로 이 순서를 적용하지 않는다.
...
5. 테스트와 구현이 함께 staged 돼 있으면 테스트 파일만 먼저 `test` 커밋으로 분리한 뒤 구현을 커밋한다.
