...
- 테스트 DB는 `src/test/resources/application-test.yml`의 `jdbc:tc:` URL로 Testcontainers가 띄우는 PostgreSQL 컨테이너 하나를 테스트 JVM 전체가 공유한다. `@DataJpaTest`에 `@AutoConfigureTestDatabase`를 붙이지 않는다 — Spring Boot는 `jdbc:tc:` URL로 잡힌 DataSource를 임베디드 DB로 교체하지 않는다. 스키마는 `ddl-auto: create`로 컨텍스트 기동 시에만 재생성한다 — `create-drop`이면 컨텍스트 캐시(기본 32개)에서 축출돼 닫히는 컨텍스트가 공유 컨테이너의 테이블을 지워, 살아 있는 다른 컨텍스트의 테스트가 `relation does not exist`로 깨진다. 컨텍스트가 늘어 캐시 상한을 넘기는 순간에만 드러나는 문제라 새 `@SpringBootTest`·`@WebMvcTest` 구성을 추가할 때 유의한다. 다른 컨텍스트의 `create-drop`과 섞이면 안 되는 스키마 검증은 `migration/FlywaySchemaValidationTest`처럼 DB 이름이 다른 `jdbc:tc:` URL을 지정해 별도 컨테이너를 쓴다.
