...
# BEDROCK_MODEL은 application.yml에 기본값이 있어 선택 사항이다.
: "${BEDROCK_REGION:?BEDROCK_REGION이 필요합니다.}"
: "${BEDROCK_ACCESS_KEY_ID:?BEDROCK_ACCESS_KEY_ID가 필요합니다.}"
: "${BEDROCK_SECRET_ACCESS_KEY:?BEDROCK_SECRET_ACCESS_KEY가 필요합니다.}"
...
jq \
	...
	--arg bedrock_access_key_id "${BEDROCK_ACCESS_KEY_ID:-}" \
	--arg bedrock_secret_access_key "${BEDROCK_SECRET_ACCESS_KEY:-}" \
				...
				if .name == $container then
					...
					| .environment = (
						...
						[
							...
							{"name": "BEDROCK_ACCESS_KEY_ID", "value": $bedrock_access_key_id},
							{"name": "BEDROCK_SECRET_ACCESS_KEY", "value": $bedrock_secret_access_key}
						] +
						...
						# 미설정 시 application.yml의 기본 모델을 쓰도록 주입 자체를 생략한다
						(if $bedrock_model == "" then
							[]
						 else
							[{"name": "BEDROCK_MODEL", "value": $bedrock_model}]
						 end)
					)
					| .image = $image
				else
